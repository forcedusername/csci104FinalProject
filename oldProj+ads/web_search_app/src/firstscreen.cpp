...
*/
    ...
    advertisements=new ad_display (advertfile);
...
void FirstScreen::print_to_list(vector<string> queryMatches, QListWidget *listWidget){
    ...
    advertisements->queryVector=getQueryVector (advertisements->query,results);

    for (unsigned int f=0; f<advertisements->queryVector.size();f++){
        ss.str (string())
        ss.clear();
        ss<<advertisements->queryVector[f];
