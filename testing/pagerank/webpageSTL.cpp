...
//you have to parse first before this works.
void WebPage:: count_words (string query){
	//cout<<"Line Vector size is: "<<lineVector.size()<<endl;
	for (unsigned int i=0; i<lineVector.size();i++){
		string explorer = lineVector [i];
		//char* replace= new char [explorer.size()+1];
		...
		while (j<explorer.length()){
			...
			if (explorer [j]=='#'){
				...
				if (explorer [j]==' '){
					...
					if ( (explorer [j]<='Z'&&explorer[j]>='A')||(explorer [j]<='z'&&explorer [j]>='a')){
						//j++;
						int n=0;
						for (unsigned int f=0; f<query.size(); f++){
							if (explorer [j++]==query[f]){
								n++;
							}
						}
						if (n==query.size()){
							//this means that the query and where you're at match
							num_headers++;
						}
					}
				}
